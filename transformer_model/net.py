...
class Transformer(nn.Module):
    ...
    def forward(self, enc_input: torch.Tensor, dec_input: torch.Tensor) -> torch.Tensor:
        ...
        print(f'!@#!@#!@# orig_src_key_padding_mask.size: {orig_src_key_padding_mask.size()}')
        ...
        src_key_padding_mask = torch.arange(x_enc_embed.size()[1]) > src_padding_length[:,None].long()
        ...
        print(f'src_key_padding_mask.size: {src_key_padding_mask.size()}')
        print(f'tgt_key_padding_mask.size: {tgt_key_padding_mask.size()}')
